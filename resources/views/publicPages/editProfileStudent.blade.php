    ...
    <div class="page-add-post">
      ...
      <div class="container">
            <div class="form-addPost">
              <form method="post" action="{{route('updateProfileStudent' , $user->id)}}" enctype="multipart/form-data">
                @csrf
                  ...
                  <div class="row">
                    <div class="col-md">
                      ...
                      <input
                              ...
                              value="{{$user->image}}"
                      />
                      <img src="{{URL::asset('image/student') .'/'. $user->image}}" alt="يرجى ادخال صورتك الشخصية" style="width: 80px;height: 80px;">
                    </div>
                    <div class="col-md">
                      <label>حدد المستوى التعليمي</label>
                      <div class="input-group">
                        <select  class="form-control" name="level_id" required>
                          <option selected disabled value="">أختر المستوى</option>
                          @foreach($level as $level_user)
                            <option value="{{$level_user->id}}">{{$level_user->name}}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
